...
$stmt = $conn->prepare("
   SELECT b.*, c.quantity FROM cart c JOIN books b ON c.book_id = b.id WHERE c.user_id = ?
...
<p><strong>In Stock:</strong> <?= htmlspecialchars($book['stock']) ?> left</p>
<?php if ($book['quantity'] > $book['stock']): ?>
    <p style="color: red;">⚠ Only <?= $book['stock'] ?> left in stock</p>
<?php endif; ?>
